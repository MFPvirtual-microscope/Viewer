...
//	jrscpProg.js is part of the "Multifocal-plane Virtual Microscope", which is an internet web-based program that
...
//		- 12 javascript files (including jrscpProg.js)
//		- three PHP files
...
function prgInitWnd() {
	...
	var cpyRtBoxLeft = Math.round((window.innerWidth - 455)/2);
	var cpyRtBoxTop = Math.round((window.innerHeight - 266)/2);
	...
	document.getElementById("menuXYVisCheckBx").checked = true;
		...
		//   ** Microsoft edge plays bell when loading (even though warnBox.style.display = "none"), prgMenuResizeSetBell) 
	...
	prgMenuResize(0);  // need to check menu size after reading cmdline & adjusting "back to slide list"
	...
	glbInfoBxDefHt = window.innerHeight - glbInfoBxDefTop - 6;  // default maximum height of infoBoxes
	return;
	}
...
function prgReInitWnd() {
	...
	if (Number.isNaN(dbMaxF) || (dbMaxF == 1)) {  // don't display if turned-off by user on previous slide
		...
		document.getElementById("menuFVisCheckBx").checked = true;
		prgMenuResize(0);  // menu size might have changed
...
function prgIntroResize(){
	const minTxtSz = 30;
	const maxTxtSz = 100;
			// set minimum values for horizontal & vertical spacing
	const dblHorMargin = 40;  // twice minimal horizontal margin of 20px
	const minMarg = 4;  // minimum vertical margin in pixels
	var extBot = 10;  // extra vertical space for calculating vertical decRatio
	if (!Number.isNaN(glbInSIntroBot)) { extBot = glbInSIntroBot; }
	var extTop = 0;  // extra space at top (e.g., to avoid overlap with sldSelBox)
	if (!Number.isNaN(glbInSIntroTop)) { extTop = glbInSIntroTop; }
		// set-up IntroPage node parameters
			// It is unclear why we need extWidth, but we get scrollbars if 
			//		we don't subtract this amount from pgWidth
			//	For FireFox, we need extWidth = 9px for Diana's laptop monitor and
			//		8px for the extra monitor
			//	For Chrome, we need 9px for both monitors
	var extWidth = 9;  // we seem to overcalculate the width by this amount 
			// get node for IntroPage
	var pgNode = document.getElementById("IntroPage");
	if (pgNode == null) {
		alert("prgIntroResize(): Cannot get node for \"IntroPage\"; "
				+ "cannot setup Intro page.\n\nPlease report this error");
		return;
		}
			// set IntroPage location & size
	var menuHt = 68;  // 2 * height of single row of menu
	var pgWidth = window.innerWidth - extWidth;
	...
	var pgHeight = window.innerHeight - menuHt - 1;
	pgNode.style.height = pgHeight + "px";
	pgNode.style.top = menuHt + "px";
	pgNode.style.width = pgWidth + "px";
			// get IntroPage element nodes
	var txtNode = document.getElementById("intPgTxt");
	if (txtNode == null) {
		alert("prgIntroResize(): Cannot get node for \"intPgTxt\"; "
				+ "cannot setup Intro page.\n\nPlease report this error");
		return;
		}
	var imgNode = document.getElementById("intPgLogo");
	if (imgNode == null) {
		alert("prgIntroResize(): Cannot get node for \"intPgLogo\"; "
				+ "cannot setup Intro page.\n\nPlease report this error");
		return;
		}
			// set-up default text (title) node parameters
	var txtSz = maxTxtSz;
	txtNode.style.fontSize = txtSz + "px";
	var txtHt = txtNode.offsetHeight;
	var txtWd = txtNode.offsetWidth;
			// set-up default Logo node parameters
	var imgHt = glbInSLogoHt;  // height of logo
	if (Number.isNaN(imgHt)) { imgHt = 500; }  // default value for imgHt
	imgWd = glbInSLogoWdth;  // width of logo
	if (Number.isNaN(imgWd)) { imgWd = 500; }  // default value for imgWd
			// imgMid & imgDblHalf were added to allow for off-center logo
			//	imgMid determines the location (in pixels from left edge) within
			//		the logo that will align with the center of IntroPage
			//	imgDblHalf is the width of a rectangle centered on the middle IntroPage
			//		that contains the entire logo
	var imgMid = glbInsLogoCntr;  // distance (in pixels) to point of logo that should be in the center of the page
	if (Number.isNaN(imgMid)) { imgMid = 250; }  // default is to center 500x500px image
	var imgDblHalf = imgMid * 2;  // width of image + extra space to center image
	if (imgMid < (imgWd/2)) { imgDblHalf = (imgWd - imgMid) * 2; }
			// calculate text-to-image ratios
			//	need to keep relative sizes of title text & logo equal
			//	if isImgLimit == true, logo will determine size of text
			//	if isImgLimit == false, text will determine size of logo
	var ratioTxtImg = txtSz/imgWd;
	var ratioImgTxt = imgWd/txtSz;
	var isImgLimit = false;
			// set title+logo width
	var maxWd = txtWd;
	if (imgDblHalf > txtWd) {
		maxWd = imgDblHalf;
		isImgLimit = true
	...
	maxWd += dblHorMargin;
	var curMargin = 20;
	var decRatio = 1;  // amount that size needs to decrease
	var decWdRatio = 1;
			// WIDTH:  fit introPage to window width
	if (maxWd > pgWidth) {  // need to decrease Intro page size
		decRatio = pgWidth/maxWd;
		txtSz = Math.floor(txtSz * decRatio);
		if (txtSz < minTxtSz) {  // minimum  font-size for title
			txtSz = minTxtSz;
			decRatio = minTxtSz/maxTxtSz;
			isImgLimit = false;
			}
				// decWdRatio is the decrease in size due to width limitations
				//	it is needed (below) when both width & height are limiting
				// decWdRatio was added ex post facto; the code probably would be
				//	have been clearer if I'd used separate decRatio's for 
				//	horizontal & vertical sizing, but (except for bugs) the code already
				//	was written.
		decWdRatio = decRatio;
				// isImgLimit is true if the logo width-limiting
		if (isImgLimit) {
				// logo size is limiting: get new logo parameters
			imgHt = Math.floor(imgHt * decRatio);  // new logo height
			imgWd = Math.floor(imgWd * decRatio);  // new logo width
			imgMid = Math.floor(imgMid * decRatio);  // new midpoint for logo
				// set txtSz using logo size
			txtSz = Math.floor(ratioTxtImg * imgWd);
			if (txtSz < minTxtSz) {
				txtSz = minTxtSz;
				isImgLimit = false;}
			}
				// isImgLimit is false if the title-text width-limiting
		if (!isImgLimit) {
				// text size is limiting:  txtSz already is set
				// get new logo parameters using text size
			imgMid = Math.floor((imgMid/imgWd) *  txtSz * ratioImgTxt);
			imgHt = Math.floor((imgHt/imgWd) * txtSz * ratioImgTxt);
			imgWd = Math.floor(txtSz * ratioImgTxt);
			}
		txtNode.style.fontSize = txtSz + "px";
		txtWd = txtNode.offsetWidth;
		txtHt = txtNode.offsetHeight;
		}  // end if width is limiting
			// check total height
			// It isn't clear why we need to add an extra line of text to totHt calculation
			//	see notes in Box03 version of Viewer
	var totHt = extTop + txtHt + imgHt + (3 * curMargin) + extBot;  // current height of page
			// HEIGHT:  screen too short; decrease size of txt & img
	if (totHt > pgHeight) {  
		decRatio = (pgHeight - (extTop + extBot))/(totHt - (extTop + extBot));
				// get new title-text parameters
		txtSz = Math.floor(txtSz * decRatio);
		if (txtSz < minTxtSz) {
			txtSz = minTxtSz; 
				// division by decWdRatio adjusts for prior width-dependent change in txtSz
			decRatio = (txtSz/maxTxtSz)/decWdRatio;
			}
		txtNode.style.fontSize = txtSz + "px";
		txtWd = txtNode.offsetWidth;
		txtHt = txtNode.offsetHeight;
				// get new logo parameters
		imgMid = Math.floor(imgMid * decRatio);  // new midpoint for logo
		imgHt = Math.floor(imgHt * decRatio);  // new logo height
		imgWd = Math.floor(imgWd * decRatio);  // new logo width
		}  // end if height is limiting
			// new vertical margins
	curMargin = Math.floor((pgHeight - (extTop + txtHt + imgHt + extBot))/3);
	if (curMargin < minMarg) { curMargin = minMarg; }
			// set style of IntroPage nodes
	txtWd = txtNode.offsetWidth;
	var curLeft = Math.floor((pgWidth - txtWd)/2);
	txtNode.style.left = curLeft + "px";
	txtNode.style.top = (extTop + curMargin) + "px";
	curLeft = Math.floor((pgWidth/2) - imgMid);
	imgNode.style.height = imgHt + "px";
	imgNode.style.width = imgWd + "px";
	imgNode.style.left = curLeft + "px";
	imgNode.style.top = (extTop + txtHt + (2*curMargin)) + "px";
	return;
	}
...
function prgWindowResize() {
	...
	// On 4/09/22:  changed prgMenuResize() to take an argument (mnuOffset), which is an integer by which
	//	scrWidth is decreased if a horizontal scrollbar is displayed, apparently because of a rounding error 
	//	when calculating the window's innerWidth or the menu's width.  This bug only appears when the entire
	//	menu is on a single line and only with certain computer monitors (e.g., Diana's laptop monitor, but
	//	not the extra monitor attached to Diana's laptop).  If, after prgMenuResize() finishes creating the 
	//	menu with mnuOffset = 0, a scrollbar is detected (by comparing window.innerHeight with 
	//	body.parentNode.scrollHeight), the prgMenuResize() calls itself with mnuOffset == 1.  This seems
	//	to correct for the error in calculating the menu width or window width and avoids display of a
	//	scrollbar.
function prgMenuResize(mnuOffset) {
	var i;
	var newMenuHt;
	...
	var scrWidth = window.innerWidth - 4 - mnuOffset;  // width of screeen less 2px border
	...
	glbMenuTotHt = newMenuHt;
			// if rounding error causes horizontal scrollbar
			//  re-call prgMenuResize() with offset to correct for rounding error
	if ((mnuOffset == 0) && (window.innerHeight != document.body.parentNode.scrollHeight)) {
		prgMenuResize(1);
		}
...
// prgInSpInit() sets-up the institution-specific ("branding") aspects HTML page.  This function was added on 3/21/22.
//	Most of the variables referenced by this function are found in jrscpInstSpcGlobal.js

function prgInSpInit() {
	var tmpNode = null;
	var tmp2Node = null;
	var parNode = null;
	var i;
	...
	if ((glbInSTitle != null) && (glbInSTitle != "")) {
		...
		if (tmpNode == null) {
			alert("prgInSpInit(): could not get node for <title>.\n\nPlease report this error");
			}
	...
	if ((glbInSFavIco != null) && (glbInSFavIco != "")) {
		...
		if (tmpNode == null) {
			alert("prgInSpInit(): could not create <link> for \"favicon\" (\"" + glbInSpFavIco 
						+ "\").\n\nPlease report this error");
			}
		else {
			tmpNode.rel = "icon";
			...
			if (document.head.appendChild(tmpNode) == null) {
				alert("prgInSpInit(): could not append node for \"favicon\" <link> (\"" + glbInSpFavIco 
		...
		alert("prgInSpInit(): no \"favicon\" was found.\n\nPlease report this error");
			...
			// Intro page title & logo
	if ((glbInSIntroTitle != null) && (glbInSIntroTitle != "")) {
		tmpNode = document.getElementById("intPgTxt");
		if (tmpNode == null) {
			alert("prgInSpInit(): could not find node (\"intPgTxt\") for title on Intro page."
						+ "\n\nPlease report this error");
			}
		else { tmpNode.innerHTML = glbInSIntroTitle; }
		}
	if ((glbInSIntroLogo != null) && (glbInSIntroLogo != "")) {
		tmpNode = document.getElementById("intPgLogo");
		if (tmpNode == null) {
			alert("prgInSpInit(): could not find node (\"intPgLogo\") for institution "
						+ "logo on Intro page.\n\nPlease report this error");
			}
		else { tmpNode.src = glbInSIntroLogo; }
		}
			// copyright box
				// copyright box title (header)
	if ((glbInSCpyRtTitle != null) && (glbInSCpyRtTitle != "")) {
		tmpNode = document.getElementById("cpyRtTitle");
		if (tmpNode == null) {
			alert("prgInSpInit(): could not find node (\"cpyRtTitle\") for copyright-box "
						+ "title.\n\nPlease report this error");
			}
		else {
			tmpNode.innerHTML = glbInSCpyRtTitle;
			tmpNode.style.top = glbInSCpyRtHdrTop + "px";
			tmpNode.style.left = glbInSCpyRtHdrLeft +"px";
			tmpNode.style.width = glbInSCpyRtHdrWd + "px";
			}
		}
				// copyright box logo
	if ((glbInSCpyRtLogo != null) && (glbInSCpyRtLogo != "")) {
		tmpNode = document.getElementById("cpyRtLogo");
		if (tmpNode == null) {
			alert("prgInSpInit(): could not find node (\"cpyRtLogo\") for institution logo in "
						+ "copyright box.\n\nPlease report this error");
			}
		else {
			tmpNode.src = glbInSCpyRtLogo;
			tmpNode.style.height = glbInSCpyRtLogoHt + "px";
			tmpNode.style.width = glbInSCpyRtLogoWd + "px";
			}
		}
				// copyright box text
	if ((glbInSCpyRtTxt != null) && (glbInSCpyRtTxt != "")) {
		tmpNode = document.getElementById("cpyRtTxt");
		if (tmpNode == null) {
			alert("prgInSpInit(): could not find node (\"cpyRtTxt\") for text in "
						+ "copyright box.\n\nPlease report this error");
			}
		else {
			tmpNode.innerHTML = glbInSCpyRtTxt;
			tmpNode.style.bottom = glbInSCpyRtTxtBot + "px";
			tmpNode.style.left = glbInSCpyRtTxtLeft + "px";
			tmpNode.style.width = glbInSCpyRtTxtWd + "px";
			if (glbInSisCpyRtShad) {
				tmpNode.style.textShadow = glbInSCpyRtTxtShad;
//				"-1px -1px 4px white, "
//						+ "-1px 1px 4px white, "
//						+ "1px -1px 4px white, "
//						+ "1px 1px 4px white, "
//						+ "0px 0px 8px rgba(248,253,255,0.5)";
				tmp2Node = document.createElement("DIV");
				if (tmp2Node == null) {
					alert("prgInSpInit(): could not create whitefade <DIV> for copyright box text."
								+ "\n\nPlease report this error."); 
					}
				else {
					parNode = document.getElementById("cpyRtBox");
					if (parNode == null) {
						alert("prgInSpInit(): could not find node (\"cpyRtBox\") for copyright box."
									+ "c\n\nPlease report this error");
						}
					else {
						tmp2Node.className = "cpyRtWhtfadeClass";
						tmp2Node.style.bottom= glbInSCpyRtFadeBxBot + "px";
						tmp2Node.style.height= glbInSCpyRtFadeBxHt + "px";
						tmp2Node.style.left = glbInSCpyRtFadeBxLeft + "px";
						tmp2Node.style.width = glbInSCpyRtFadeBxWd + "px";
						tmp2Node.style.backgroundColor = glbInSCpyRtFadeBxBkg;
						tmp2Node.style.opacity = glbInSCpyRtFadeBxOpac;
						parNode.insertBefore(tmp2Node,tmpNode);
						}
					}
				}
			}
			...
			// button Logo: 30px x 30px png; can specify background color
	const btnList =  [
						"sldSelLogo",
						"menuToLstLogo"
						];
	const btnLstSz = btnList.length;
	if ((glbInSBtnLogo != null) && (glbInSBtnLogo != "")) {
		for (i = 0; i < btnLstSz; i++) {
			tmpNode = document.getElementById(btnList[i]);
			if (tmpNode == null) {
				alert("prgInSpInit(): could not find node for logo: \"" + btnList[i] + 
						+ "\".\n\nPlease report this error");
				}
			else {
				tmpNode.src = glbInSBtnLogo;
				if  ((glbInSBtnBkgClr != null) && (glbInSBtnBkgClr != "")) {
					tmpNode.style.backgroundColor = glbInSBtnBkgClr;
					}
				}
			}
// START HERE
	...
	prgBuildAbtMnu();  // build "About" menu, which is institution-specific
...
function prgBuildAbtMnu() {
	var mnuNode = document.getElementById("mnuAboutContent");
	var i;
	if (mnuNode == null) {
	...
	if ((glbInSAbtLst == null) || (glbInSAbtLst.length == 0)) {
		...
		if ((mnuNode == null) || !(mnuNode.className.includes("menuDrpDwnContainer"))) {
			...
			alert("This version of the Multifocal Plane Virtual Microscope is missing the \"About\" menu, "
	...
	for (i = 0; i < lstSz; i++) {
		...
		if (glbInSAbtLst[i].mnuClk != null) {
			itmNode.onclick = glbInSAbtLst[i].mnuClk;
			itmNode.addEventListener("touchstart",tchClkBtnDwn);  // these need to be checked/debugged
			itmNode.addEventListener("touchend",tchClkBtnUp);
			itmNode.addEventListener("touchcancel",tchClkBtnUp);	
			}
...
function sqlSldBasics(ajxReq) {
	...
	if (sldData.sldRoot == null) {
		alert("sqlSldBasics():  Unable to get location of slide from server."
				+ "\n  Please close the Virtual Microscope Viewer, and report the problem.");
		return;
		}
	...
	prgMenuResize(0);  // need to check menu size: may have removed "menuFP" from main menu and may need to adjust for filename
...
function ajxConnectFail() {
	...
	txtStr += "\n  The Virtual Microscope will not function without a server connection.";
	...
	if (strI == 0) { // ajxStr begins with "!!!!" - message includes both errors & data
		...
		if (strJ < 4) {   // missing end of combined error & data string
			alert(funcStr + ":  Data from server is corrupted (error messages not set-off by \"!!!!\")."
						+ "\n\n  Please close the Virtual Microscope Viewer and report the error.");
			return ("");
			}
...
function prgToSB(musEvt) {
	...
	if ((glbInSSldBxURL == null) || (glbInSSldBxURL == "")) {
		alert("prgToSB(): invalid URL for SlideBox.  Cannot use SlideBox to select the virtual slide.\n\nPlease report this error.");
		return;
		}
	if (isShftKey) { window.open(glbInSSldBxURL,"_blank"); }
	else if (isCntrlKey) { window.location.assign(glbInSSldBxURL); }
	else { window.location.replace(glbInSSldBxURL); }
	return;
	}
