...
//	jrscpSldView.js is part of the "Multifocal-plane Virtual Microscope", which is an internet web-based program that
...
//		(jrscpSldView.js) is part of the Viewer.  Currently, the Viewer consists 
//		of 18 principal files and other supplementary files:
//		- one HTML file.
//		- two cascading style sheets
//		- 12 javascript files (including jrscpSldView.js)
//		- three PHP files
//	jrscpSldView.js contains javascript functions that are involved in displaying the 
...
function sldImgLoaded(idStr) {
	...
	if (Number.isNaN(sldVwF)) {
		alert("sldImgLoaded() could not read imgNode.id (\"" + idStr + "\")." 
				+ "\n  You probably should close the Virtual Microscope, as this probably is a fatal error."
				+ "\n  Please report this bug.");
		return;
		}
	if (Number.isNaN(sldVwZ)) {
		alert("sldImgLoaded() could not read imgNode.id (\"" + idStr + "\")." 
				+ "\n  You probably should close Virtual Microscope, as this probably is a fatal error."
				+ "\n  Please report this bug.");
		return;
		}
