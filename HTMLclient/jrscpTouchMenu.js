...
//	jrscpTouchMenu.js is part of the "Multifocal-plane Virtual Microscope", which is an internet web-based program that
...
//		- 12 javascript files (including jrscpTouchMenu.js)
//		- three PHP files
...
//	On 12/02/19, jrscpTouch.js was split into jrscpTouchMenu.js and jrscpTouchMain.js because
...
function tchMenuUnblock() {
	...
	var btnLst = [
				...
				"menuClearDestArr",
				"menuChgSet",
					// ABOUT MENU => get from glbInSLst[]; see below
	...
	for (i = 0; i < lstSz; i++ ) {
		...
		if (btnNode == null) {
			alert("tchMenuUnblock():  could not get node for \"" + btnLst[i] 
		...
		btnNode.style.color = "";
			...
			// reset background and text colors for "About" menu items (5/02/22)
	lstSz = glbInSAbtLst.length;
	var btnId;
	for (i = 0; i < lstSz; i++) {
		btnId = glbInSAbtLst[i].mnuId;
		if ((btnId == null) || (btnId == "")) { continue; }
		btnNode = document.getElementById(btnId);
		if (btnNode == null) {
			alert("tchMenuUnblock():  could not get node for \"" + btnId 
					+ "\".\n   Could not reset text and background colors for this item.");
			continue;
			}
		btnNode.style.backgroundColor = "";
	...
	for (i = 0; i < lstSz; i++ ) {
		btnNode = document.getElementById(btnLst[i]);
		if (btnNode == null) {
			alert("tchMenuUnblock():  could find \"" + btnLst[i] + "\".\n   Could not reset text and background colors for this item.");
			continue;
			}
		btnNode.style.color = "";
		}
...
function tchClkBtnInit() {
	...
	var btnLst = [
				...
				"menuClearDestArr",
				"menuChgSet",
				// for "About" menu items, see (in jrscpProg.js) prgBuildAbtMnu()
				];
	var lstSz = btnLst.length;
	for (i = 0; i < lstSz; i++ ) {
		btnNode = document.getElementById(btnLst[i]);
...
function tchClkBtnUp(tchEvt) {
	...
	if (extraN.node2 != null) {
		...
		extraN.node2.style.color = "black";
			...
			// if btn is an "About" menu item, use glbInSAbtLst to call onclick effector function
			//	NOTE:  5/02/22 THIS CODE HAS NOT YET BEEN TESTED/DEBUGGED !!!!
			//	old version of this function is in InstSpc01_220502
	var lstSz = glbInSAbtLst.length;
	for (i = 0; i < lstSz; i++){
		if (btnId == glbInSAbtLst[i].mnuId) {
			glbInSAbtLst[i].mnuClk;   // I'm not sure this will work; I think it will invoke the function
			return;  // invoking the "onClick" effector function is the last action of tchClkBtnUp() before returning
			}
		}
			// "About" menu items already handled & return (5/02/22); see above
			// call "onClick" effector functions
	switch (btnId) {
		case "menuSldNmVisCheckBx" : 
		case "menuSldNmVisLbl" : 
		...
		case "menuChgSet": chgBuildAllSetBox();
			break;
		// see above for "About" menu items
